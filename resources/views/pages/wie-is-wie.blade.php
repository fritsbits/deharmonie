...
<div style="max-width: 72rem; margin: 0 auto; padding: 3.5rem 1.5rem;">
    ...
    <h2 style="font-family: var(--font-sans); font-size: clamp(1.5rem, 2.5vw, 2.25rem); font-weight: 900; color: var(--color-brand-blue); margin: 0 0 1.5rem; line-height: 1.1;">{{ __('pages.team_staff') }}</h2>

    <div style="border-top: 1px solid var(--color-brand-border-blue);">
        @foreach ($staf as $categorie)
            <div class="wie-rij" style="padding: 1.25rem 0; border-bottom: 1px solid var(--color-brand-border-blue);">
                <h3 class="wie-label" style="font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: var(--color-brand-blue); font-family: var(--font-sans); margin: 0; padding-top: 0.2rem;">
                    {{ $categorie->naam }}
                </h3>
                <div class="wie-namen">
...
<div style="max-width: 72rem; margin: 0 auto; padding: 3.5rem 1.5rem;">
    ...
    <h2 style="font-family: var(--font-sans); font-size: clamp(1.5rem, 2.5vw, 2.25rem); font-weight: 900; color: var(--color-brand-green-dark); margin: 0 0 1.5rem; line-height: 1.1;">{{ __('pages.governance') }}</h2>

    <div class="bestuur-grid">
        @foreach ($governance as $categorie)
            <div>
                <h3 style="font-size: 1.375rem; font-weight: 800; color: var(--color-brand-dark); font-family: var(--font-sans); margin: 0 0 1rem; padding-bottom: 0.75rem; border-bottom: 2px solid var(--color-brand-border-green); line-height: 1.2;">
                    {{ $categorie->naam }}
                </h3>
                @foreach ($categorie->leden as $lid)
                    <p style="font-size: 1rem; color: var(--color-brand-dark); line-height: 1.7; margin: 0;">
