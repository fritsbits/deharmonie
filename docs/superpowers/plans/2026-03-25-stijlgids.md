# Stijlgids Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a hidden style guide page at `/stijlgids` showing all De Harmonie design tokens and UI components in one labeled, named reference page.

**Architecture:** Single static Blade view (`resources/views/stijlgids.blade.php`) extending a minimal layout (`resources/views/layouts/stijlgids.blade.php`). No controller, no Livewire. Route added to `routes/web.php` outside locale groups but wrapped in `locale:nl` middleware. Two-column layout: sticky sidebar with 12 anchor links, scrollable content with Zone 1 (building blocks) and Zone 2 (full section previews).

**Tech Stack:** Laravel 13, Blade, Tailwind v4 (via `@vite`), inline styles only (same pattern as all other views in this project). No JavaScript needed.

**Spec:** `docs/superpowers/specs/2026-03-25-stijlgids-design.md`
...
| Action | File | Purpose |
|---|---|---|
| Modify | `routes/web.php` | Add `/stijlgids` route with `locale:nl` middleware |
| Create | `resources/views/layouts/stijlgids.blade.php` | Minimal layout shell (fonts, CSS, no nav/footer) |
| Create | `resources/views/stijlgids.blade.php` | Style guide page — all 12 sections |
| Create | `tests/Feature/StijlgidsTest.php` | HTTP tests for the style guide page |

---

## Task 1: Route + failing test

**Files:**
- Modify: `routes/web.php`
- Create: `tests/Feature/StijlgidsTest.php`
...
```bash
php artisan test --filter=StijlgidsTest
```
...
In `routes/web.php`, append after the FR group (line 29):
...
- [ ] **Step 4: Run tests again — expect view-not-found error now (not 404)**

```bash
php artisan test --filter=StijlgidsTest
```

Expected: still failing, but now with "View [stijlgids] not found" — confirms the route is wired up correctly.

---

## Task 2: Minimal layout shell

**Files:**
- Create: `resources/views/layouts/stijlgids.blade.php`
...
- [ ] **Step 2: Create a minimal stijlgids view to verify layout works**

Create `resources/views/stijlgids.blade.php` (skeleton only — will be filled out in later tasks):
...
- [ ] **Step 3: Run all three tests — expect all to pass**

```bash
php artisan test --filter=StijlgidsTest
```

Expected: 3 PASS. The page loads, all 12 section IDs exist, noindex is set.

- [ ] **Step 4: Commit**

```bash
git add routes/web.php resources/views/layouts/stijlgids.blade.php resources/views/stijlgids.blade.php tests/Feature/StijlgidsTest.php
git commit -m "feat: add /stijlgids route, layout, skeleton view, and tests"
```

- [ ] **Step 5: Verify visually in browser**

Open `https://harmonie.test/stijlgids` — expect the two-column layout with a sidebar and 12 placeholder sections.

---

## Task 3: Zone 1 — Kleurenpalet & Typografie

**Files:**
- Modify: `resources/views/stijlgids.blade.php` (sections 1–2)

Replace the `#kleurenpalet` and `#typografie` placeholder sections with the full content below.

- [ ] **Step 1: Replace `#kleurenpalet` section**

Replace the placeholder `<section id="kleurenpalet">` block with:
...
- [ ] **Step 2: Replace `#typografie` section**

Replace the placeholder `<section id="typografie">` block with:
...
Open `https://harmonie.test/stijlgids` — expect color swatches and all typography sizes displayed with labels.

- [ ] **Step 5: Commit**

```bash
git add resources/views/stijlgids.blade.php
git commit -m "feat: stijlgids zone 1 — kleurenpalet en typografie"
```

---

## Task 4: Zone 1 — Knoppen, Formulieren, Badges

**Files:**
- Modify: `resources/views/stijlgids.blade.php` (sections 3–5)

- [ ] **Step 1: Replace `#knoppen` section**
...
- [ ] **Step 2: Replace `#formulieren` section**
...
```bash
git add resources/views/stijlgids.blade.php
git commit -m "feat: stijlgids zone 1 — knoppen, formulieren, badges"
```

---

## Task 5: Zone 2 — Navigatiebalk & Hero

**Files:**
- Modify: `resources/views/stijlgids.blade.php` (sections 6–7)

- [ ] **Step 1: Replace `#navigatie` section**

Static HTML copy — no `route()` calls, `href="#"` throughout:
...
```bash
git add resources/views/stijlgids.blade.php
git commit -m "feat: stijlgids zone 2 — navigatiebalk en hero"
```

---

## Task 6: Zone 2 — Activiteitenlijst & Detail sidebar

**Files:**
- Modify: `resources/views/stijlgids.blade.php` (sections 8–9)

- [ ] **Step 1: Replace `#activiteitenlijst` section**
...
- [ ] **Step 2: Replace `#activiteit-detail` section**
...
```bash
git add resources/views/stijlgids.blade.php
git commit -m "feat: stijlgids zone 2 — activiteitenlijst en detail sidebar"
```

---

## Task 7: Zone 2 — Registratieformulier, Diensten & Voettekst

**Files:**
- Modify: `resources/views/stijlgids.blade.php` (sections 10–12)

- [ ] **Step 1: Replace `#registratieformulier` section**
...
- [ ] **Step 2: Replace `#diensten` section**
...
- [ ] **Step 3: Replace `#voettekst` section**

Static HTML copy of the footer — no dynamic `route()` calls:
...
```bash
git add resources/views/stijlgids.blade.php
git commit -m "feat: stijlgids zone 2 — formulier, diensten, voettekst (complete)"
```

- [ ] **Step 7: Final visual check**

Open `https://harmonie.test/stijlgids` and scroll through all 12 sections. Verify:
- Sidebar links scroll to correct anchors
- Colors display with swatches and token names
- Typography shows all size variants
- Buttons render correctly
- Badges and banners show correct colors
- Section previews (nav, hero, sidebar card, footer) match the live site
...
- `https://harmonie.test/stijlgids` is accessible
- All 12 sections are visible and correctly styled
- The page is not linked from nav, footer, or any public page
